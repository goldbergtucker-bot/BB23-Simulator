...
(() => {
  ...
  castGrid.addEventListener("input",e=>{const el=e.target,h=state.houseguests.find(x=>x.id===el.dataset.id);if(!h)return;if(el.dataset.field){h[el.dataset.field]=el.value;const card=el.closest('.cast-card');const title=card?.querySelector('.cast-name');if(title)title.textContent=name(h);}if(el.dataset.rating){h.ratings[el.dataset.rating]=Number(el.value);const s=el.parentElement.querySelector("strong");if(s)s.textContent=el.value;}validate();});
